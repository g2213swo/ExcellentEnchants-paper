...
public class EnchantDecapitator extends ExcellentEnchant implements Chanced, DeathEnchant {
    ...
    public boolean onKill(@NotNull EntityDeathEvent e, @NotNull LivingEntity entity, @NotNull Player killer, int level) {
        ...
        if (entityType == EntityType.WITHER_SKELETON || entityType == EntityType.WITHER) {
            ...
            Component entityName; // adventure
            if (entity instanceof Player player) {
                entityName = ComponentUtil.replace(text(this.headName), Placeholders.GENERIC_TYPE, player.getName()); // adventure
                meta.setOwningPlayer(player);
            } else {
                String texture = this.headTextures.get(entity.getType());
                if (texture == null) return false;

                entityName = ComponentUtil.replace(text(this.headName), Placeholders.GENERIC_TYPE, translatable(entityType)); // adventure
                ItemUtil.setSkullTexture(item, texture);
                meta = (SkullMeta) item.getItemMeta();
            }

            meta.displayName(entityName); // adventure
        ...
        PDCUtil.getString(skull, this.skullKey).ifPresent(type -> {
            ...
            e.getItems().forEach(item -> {
                ...
                if (drop.getType() == Material.PLAYER_HEAD) {
                    ...
                    // adventure starts
                    drop.editMeta(meta -> {
                        meta.displayName(ComponentUtil.replace(
                            text(this.headName),
